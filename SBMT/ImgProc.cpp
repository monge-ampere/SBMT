...
void ImgProc::OutputBdyIds(int *ids, int &bN)
{
	int N = boundary.size();
	bN = N;
	for (int i = 0; i != N; ++i)
...
void ImgProc::OutputTris(int3 *tri_set, int &tN)
{
	tN = avail_tria.size();
	for (int i = 0; i != tN; ++i)
	{
		tri_set[i][0] = this->avail_tria[i][0];
		tri_set[i][1] = this->avail_tria[i][2];
		tri_set[i][2] = this->avail_tria[i][1];
	}
}
