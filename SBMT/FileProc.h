	...
	int pN, uchar *gray, int gN, int(*tri_set)[3], int tN, string path)
{
	...
	for (int i = 0; i != pN; ++i)
	{
		...
		fprintf(fp, "Vertex %d %f %f %f {rgb=(%f %f %f) normal=(0 0 1.0) father=(%d)}\n"
			, i, (float)taget[i][0], (float)taget[i][1], 0.0, g01, g01, g01, i);
	}
...
inline void write2f(double(*taget)[2], int pN, string path)
{
	...
	{
		fprintf(fp, "%f,%f\n"
			, (float)taget[i][0], (float)taget[i][1]);
	}
...
inline void Ps2Txt(double(*taget)[2], int bN, string path)
{
	FILE *fp = fopen(path.c_str(), "w+");
	if (fp == NULL) 
	{
...
inline void Bdy2Txt(int *taget, int bN, string path)
{
	FILE *fp = fopen(path.c_str(), "w+");
	if (fp == NULL) {
		fprintf(stderr, "fopen() failed.\n");
...
inline void Tris2Txt(int (*taget)[3], int bN, string path)
{
	FILE *fp = fopen(path.c_str(), "w+");
	if (fp == NULL) {
		fprintf(stderr, "fopen() failed.\n");
	...
	for (int i = 0; i != bN; ++i)
	{
		fprintf(fp, "%d %d %d\n", taget[i][0] + 1, taget[i][1] + 1, taget[i][2] + 1);
	}
...
inline void readm(Pt *pts, vector<int> &ids, vector<vector<int>> &tris, const char* path)
{
	...
	while (is.getline(buffer, _MAX_PATH))
	{
		...
		if (token == "Vertex")
		{
			...
			stokenizer.nextToken();
			token = stokenizer.getToken();
			p.oci = strutil::parseString<float>(token);

			stokenizer.nextToken();
			token = stokenizer.getToken();
			p.ori = strutil::parseString<float>(token);

			stokenizer.nextToken();
			token = stokenizer.getToken();
			p.x = strutil::parseString<float>(token);

			stokenizer.nextToken();
			token = stokenizer.getToken();
			p.y = strutil::parseString<float>(token);
